...
export const OffCanvasComponent = () => {
  ...
  const displayOptions = async (inputString) => {
    try {
      const response = await fetch(
        `https://api.openweathermap.org/geo/1.0/direct?q=${inputString}&limit=5&appid=${process.env.REACT_APP_OPEN_WEATHER_API_KEY}`
      );
      if (response.ok) {
  ...
  const fetchLocation = async (location) => {
    try {
      const response = await fetch(
        `https://api.openweathermap.org/geo/1.0/direct?q=${location}&limit=6&appid=${process.env.REACT_APP_OPEN_WEATHER_API_KEY}`
      );
      if (response.ok) {
        const data = await response.json();
        const coordinates = {
